---
title: Visualizing Prophet Forecasts (And Errors) with furrr, and gganimate
author: porter
date: '2018-12-22'
...
The idea and methods of using `furrr` with `Prophet` here were not my own. I copied this methodology from a coworker who was kind enough to share this approach below which works absolute **wonders** on execution time of running multiple `Prophet` forecasts.
...
And this reminded me of a cool animation I made for myself to better understand what one of the inputs `changepoint.prior.scale` was actually doing.

Inspired by another writeup from [Len Keifer](http://lenkiefer.com/2017/08/10/animation-progress/), I figured an animated time-series would give me a good idea for comparing different outputs when I toggled this parameter. The below writeup is the process I went through to generate the gif at the top of this post.
...
It spits out fancy names like yhat because science. `yhat` just means the predicted value. It is what you're looking for if you've gotten this far. It provides some confidence intervals in the form of `yhat_upper` and `yhat_lower`.
...
Then we use `furrr::future_map()` to run a forecast model for each day in our backtest.
...
We also have a forecast generated using data up to January 1 **2018**
...
forecast_plot <- ggplot(
  forecast_df %>% filter(ds >= '2018-01-01', ds <= forecast_date + 365, forecast_date >= '2018-01-01')
  , aes(
    x = ds
  )
) +
...
anim_save(filename="~/Documents/peakmaximum/static/img/forecast_plot.fib", animation = forecast_plot)
