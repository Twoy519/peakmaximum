---
title: More Re-Creations of Lenore Tawney's Work
author: "Troy Porter"
date: '2020-02-10'
description: "Inspired by @W_R_Chase we will be reproducing more work done by Lenore Tawney"
slug: more-lenore-tawney
categories: ['ggplot','lenore_tawney','art']
tags: []
...
fray <- function(data, min_fray = 20, max_fray = 100, num_fray = 3) {
  frayer <- function(dat, start, end) {
    dat %>%
      ...
      ungroup() %>%
      mutate(
        inertia2 = sample(c(0, 1), nrow(.), replace = TRUE, prob = c(1, 1)),
...
gust <- function(data, angle = 0, force = 100, diff_mod = 100, inertia_mod = 50, jitter_min = 0.1, jitter_max = 1, jitter_mod = 1) {
  ...
  pts_new <-
    data_jittered %>%
    mutate(
      x = ifelse(inertia == 0, x + cos(a) * force, (x + (cos(a) * (force * inertia))) +
...
circle_seed <- gen_seed_cir(n_grains = 100000, wind_angle = 270, r = 50, split_mod = 2)

# generate a list of 5 circles, each one slightly larger than the previous
circles <- list(
  a = circle(100000, r = 50),
