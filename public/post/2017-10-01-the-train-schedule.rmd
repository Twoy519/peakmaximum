...
![The Train Schedule](/img/train_schedule.png)
...
M y writing style here is going to be pretty flow-ey. I'm gonna follow my thought process on what speed-bumps I faced and how I solved them. You may find this rudimentary but my goal here is actually to show the smart people out there how dumb people like me try to solve problems.

First, let's load the packages we need.

```{r fireItUp, message=FALSE, warning=FALSE, error=FALSE}
...
```{r plagiarizeMikeBostockAgain, message=FALSE, warning=FALSE, error=FALSE}
...
First let's use `knitr::kable()` and `head()` to just see what columns we have.

```{r glimpse_td, message=FALSE, warning=FALSE, error=FALSE}
...
```{r nrow_td, message=FALSE, warning=FALSE, error=FALSE}
...
```{r column_names, message=FALSE, warning=FALSE, error=FALSE}
...
```{r munge_timestamps, message=FALSE, warning=FALSE, error=FALSE}
...
```{r station_factoring, message=F, error=F, warning=F}
...
We're missing something important though, the distance the trains are traveling. How can we capture that? I bet that value is what was in the column names before we cleaned them `r emo::ji("sad")`

Good thing we made a backup of the data in its raw imported form.

`r colnames(td_backup)`

```{r munge_distance, error=F, message=F, warning=F}
...
```{r join_distance, error=F, warning=F, message=F}
...
```{r plot_try_one, error=F, message=F, warning=F}
