---
title: Generating A Logarithmic Bucketing in R
author: porter
date: '2017-12-07'
slug: generating-a-logarithmic-bucketing
...
So let's generate a lognormal distribution with some really high outliers. Credit [here](https://msalganik.wordpress.com/2017/01/21/making-sense-of-the-rlnorm-function-in-r/) for the way to do this.
...
bucketed <- impressions %>% 
  group_by(bucket) %>% 
  summarise(imps = sum(as.numeric(imps), na.rm=T)) %>%
  ungroup() %>% 
  mutate(
...
ggplot(
  ...
  labs( title='Percentage by Bucket') +
